<template>
  <q-page class="flex column q-pa-md flex-center text-center">
    ...
    />
    <template v-else>
      <div v-if="!ingresoId || (ingresoId && ingresoData.id)" class="row justify-center full-width">
        <div class="col-12 col-sm-8 col-md-6">
          <h5 class="page-title" v-if="ingresoData?.id">Editar {{ ingresoData.reason ? `"${ingresoData.reason}"` : 'ingreso'}}</h5>
          <h5 class="page-title" v-else>Crear nuevo ingreso</h5>
          <q-form
            @submit="handleSubmitIngreso"
            class="q-gutter-md"
          >
            <q-input
              filled
...
const ingresoId = route.params.id
const isLoading = ref(!!ingresoId)
...
function fetchData() {  
  ...
  api.get('ingresos/' + ingresoId)
    .then(response => {
      ingresoData.value = response.data
      ...
      if (response.data.categori_id) {
        api.get('categorias/' + response.data.categori_id).then(response2 => {
          ingresoData.value.categoryName = response2.data?.name || response.data.categori_id
          updateData.value.category = {
            label: ingresoData.value.categoryName,
      ...
      if (response.data.sub_categori_id) {
        api.get('sub-categorias/' + response.data.sub_categori_id).then(response2 => {
          ingresoData.value.subCategoryName = response2.data?.name || response.data.sub_categori_id
          updateData.value.subCategory = {
            label: ingresoData.value.subCategoryName,
...
const handleSubmitIngreso = () => {
  ...
  if (ingresoId) {
    ...
    api.put('ingresos/' + ingresoId, postData).then(response => {
      if (response.data?.status == 200) {
        ingresoData.value = response.data?.data
        $q.notify({
          type: 'positive',
  ...
  // Crear nuevo
    ...
    api.post('ingresos', postData).then(response => {
      if (response.data?.status == 200) {
        $q.notify({
          type: 'positive',
...
const handleResetForm = () => {
  updateData.value.reason = ingresoData.value?.reason || null
  updateData.value.total = ingresoData.value?.total || null
  if (ingresoData.value?.categori_id) {
    updateData.value.category = {
      label: ingresoData.value.categoryName || ingresoData.value.categori_id,
      value: ingresoData.value.categori_id,
    }
  } else { updateData.value.category = null }

  if (ingresoData.value?.sub_categori_id) {
    updateData.value.subCategory = {
      label: ingresoData.value.subCategoryName || ingresoData.value.sub_categori_id,
      value: ingresoData.value.sub_categori_id,
    }
